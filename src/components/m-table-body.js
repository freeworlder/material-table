...
class MTableBody extends React.Component {
    ...
    renderEmpty(emptyRowCount, renderData) {
        ...
        if (this.props.options.showEmptyDataSourceMessage && renderData.length === 0) {
            ...
            return (
                <TableRow
                    ...
                    <TableCell style={{ paddingTop: 0, paddingBottom: 0, textAlign: 'center' }}
                               colSpan={this.props.columns.length + addColumn} key="empty-">
            ...
            return (
                <React.Fragment>
                    {[...Array(emptyRowCount)].map((r, index) => <TableRow style={{ height: rowHeight }}
                                                                           key={'empty-' + index}/>)}
    ...
    onEditableCellClick = rowIndex => field => (/*event*/) => {
        this.setState({ selectedCell: { rowIndex, field } });
    };

    deselectCell = rowIndex => field => (value, selectNext = false) => {
        let rowData = this.props.renderData[rowIndex];
        ...
        return new Promise(resolve => {
            if (entity[field] !== value) {
                this.props
                    ...
                    .then(() => {
                        resolve();
                        this.selectNext(selectNext, rowIndex, field);
                    });
            } else {
                resolve();
                this.selectNext(selectNext, rowIndex, field);
    ...
    selectNext = (selectNext, rowIndex, field) => {
        let selectedCell = {}; //nothing selected
        if (selectNext) {
            // checking there is a next row
            const totalRows = this.props.totalRecords;
            const currentPage = this.props.currentPage;
            const rowsPerPage = this.props.pageSize;
            if (rowsPerPage * currentPage + rowIndex + 1 === totalRows) {
                // last row
                // focus back button
                const prevPaginationRef = this.props.prevButtonRef;
                prevPaginationRef &&
                prevPaginationRef.current &&
                prevPaginationRef.current.focus();
            } else if (rowIndex === rowsPerPage - 1) {
                // last row on page
                // focus on next button
                const nextPaginationRef = this.props.nextButtonRef;
                nextPaginationRef &&
                nextPaginationRef.current &&
                nextPaginationRef.current.focus();
            } else {
                selectedCell = {
                    field,
                    rowIndex: rowIndex + 1
            ...
            if (data.tableData.editing) {
                return (
                    <this.props.components.EditRow
                        ...
                        localization={{
                            ...MTableBody.defaultProps.localization.editRow, ...this.props.localization.editRow,
                        ...
                        deselectCell={this.deselectCell(index)}
                    />
                );
            } else {
                return (
        ...
        return (
            <TableBody>
                ...
                <this.props.components.FilterRow
                    ...
                    localization={{
                        ...MTableBody.defaultProps.localization.filterRow, ...this.props.localization.filterRow,
                        dateTimePickerLocalization: this.props.localization.dateTimePickerLocalization
                    }}
